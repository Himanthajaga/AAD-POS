...
$('#update-customer').on('submit', function (event) {
    ...
    if (isValidated) {
        ...
        if (customerDatabase.some(c => c.id === id)) {
            $.ajax({
                url: "http://localhost:8080/Application1_Web_exploded/customer?id="+ id + "&name=" + name + "&phone=" + contactNo + "&address=" + address,
                method: "PUT",
                success: function (response) {
...
function confirmAndDelete(customers) {
    ...
    $('#confirm-delete-btn').one('click', function () {
        customers.forEach(customer => {
            $.ajax({
                url: `http://localhost:8080/Application1_Web_exploded/customer?id=${customer.id}`,
                method: 'DELETE',
                success: function () {
